...
class ModalSettings extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    const liveDrawOnMap = JSON.parse(await AsyncStorage.getItem("liveDrawOnMap"));

    this.setState({uniqueDataEnabled: uniqueDataEnabled, liveDrawOnMap: liveDrawOnMap});
  }
  ...
  render() {
    ...
    return(
      <View style={gStyle.container}>
        ...
        >
          <Text style={styles.note}> <Text style={{color:colors.red}}>*</Text> Filtering can take some time when you open the data page.</Text>
        </View>
        <View style={{
              flexDirection: "row",
              paddingTop: "3%",
          ...
          <View style={{flex:0.2,alignItems:'flex-end'}}>
            <Switch
              onValueChange={value => {
                ...
                var changeStatusLiveDrawOnMap = navigation.getParam('changeStatusLiveDrawOnMap');
        ...
        >
          <Text style={styles.note}> <Text style={{color:colors.red}}>*</Text> If disabled, drawing is done after recording is finished. It can be disabled for application speed.</Text>
        </View>
      </View>
    )
  }
